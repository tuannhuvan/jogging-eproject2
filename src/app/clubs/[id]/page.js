...
/**
 * CLUB DETAIL PAGE - Trang chi tiết câu lạc bộ
 * 
 * Trang này hiển thị:
 * - Banner hero với hình ảnh và thông tin CLB
 * - Tabs: Giới thiệu, Thành viên, Hoạt động, Ảnh & Video
 * - Nút theo dõi CLB
 * - Chat button cộng đồng
 * 
 * Route: /clubs/[id]
 */
...
/**
 * Component trang chi tiết câu lạc bộ
 * Hiển thị đầy đủ thông tin và các hoạt động của CLB
 */
export default function ClubDetailPage() {
  // Lấy id từ URL params
  const { id } = useParams()
  // State lưu thông tin câu lạc bộ
  const [club, setClub] = useState(null)
  // State lưu danh sách thành viên
  const [members, setMembers] = useState([])
  // State lưu danh sách bài đăng
  const [posts, setPosts] = useState([])
  // State theo dõi trạng thái đang tải
  const [loading, setLoading] = useState(true)
  // State theo dõi trạng thái đã theo dõi CLB chưa
  const [isFollowing, setIsFollowing] = useState(false)
  // State lưu thông tin user hiện tại
  const [user, setUser] = useState(null)

  // Effect: Tải dữ liệu câu lạc bộ khi component mount
  useEffect(() => {
  ...
  // Hiển thị thông báo nếu không tìm thấy CLB
  if (!club) return <div className="min-h-screen flex items-center justify-center">Không tìm thấy câu lạc bộ</div>

  /**
   * Hàm lấy badge vai trò thành viên với màu sắc tương ứng
   * @param {string} role - Vai trò của thành viên
          ...
          <TabsContent value="community" className="space-y-6">
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {posts.length > 0 ? posts.map((post) => (
                   ...
                   // Ảnh demo nếu không có bài đăng
                      ...
                      <div key={i} className="relative aspect-square rounded-xl overflow-hidden group cursor-pointer shadow-md">
                           <Image
                              src={[
                                ...
                                'https://images.unsplash.com/photo-1530541930197-ff16ac7a7b2e?w=500',
                                'https://images.unsplash.com/photo-1502126324834-38f8e02d7160?w=500',
                                ...
                                'https://images.unsplash.com/photo-1486739985386-d4fae04ca6f7?w=500',
